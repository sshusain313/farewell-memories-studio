...
const GridBoard = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-1 sm:p-2 md:p-6">
      ...
      <div
        className="grid grid-cols-10 bg-white rounded-xl shadow-2xl p-1 md:p-3 w-full"
        style={{
          gap: 'var(--gap)',
          ['--gap' as any]: '1px',
          ['--pad' as any]: '4px',
          ['--cell' as any]: isDesktop
            ? 'min(calc((35vw - (var(--pad)*2) - (9 * var(--gap))) / 10), calc((100vh - (var(--pad)*2) - (9 * var(--gap))) / 10))'
            : 'calc((100vw - (var(--pad)*2) - (9 * var(--gap))) / 10)', // Mobile: prioritize width fit
